...
const url = "http://localhost:8083/api/salidapro";

class Salidapro extends Component {
    ...
    seleccionarEmpresa = (salidapro) => {
        this.setState({
            tipoModal: 'actualiza',
            form: {
                id: salidapro.id,
                cantidad: salidapro.cantidad,
                estado: salidapro.estado,
                fecha: salidapro.fecha
            }
        })
    }
    ...
    render() {
        ...
        return (
            <div className="App" >
                <br />
                <button className='btn btn-success' onClick={() => { this.setState({ form: null, tipoModal: 'insertar' }); this.modalInsertar() }}>Agregar Salidapro</button>
                <br />
                <br />
                <table className='table'>
                    ...
                    <tbody>
                        {this.state.data.map(salidapro => {
                            return (
                                <tr>
                                    <td>{salidapro.id}</td>
                                    <td>{salidapro.cantidad}</td>
                                    <td>{salidapro.estado}</td>
                                    <td>{salidapro.fecha}</td>
                                    <td>{salidapro.inventario_id}</td>
                                    <td>{salidapro.usuario_id}</td>
                                    <td>
                                        <button className='btn btn-primary' onClick={() => { this.seleccionarEmpresa(salidapro); this.modalInsertar() }}><FontAwesomeIcon icon={faEdit} /></button>
                                        {" "}
